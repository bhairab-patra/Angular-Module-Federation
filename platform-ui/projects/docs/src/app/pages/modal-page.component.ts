...
<docs-page
  ...
  <ng-container demo>
    ...
    <!-- 1. Dialog trigger cards -->
    <div class="demo-section">
      <h3 class="demo-section__title">Live Demos</h3>
      <p class="demo-section__desc">Click any card to open the corresponding dialog. Each shows a different pattern: basic, destructive confirm, success receipt, and form.</p>
      <div class="trigger-grid">

        <pui-card variant="flat" [clickable]="true" (cardClick)="basicOpen=true;cdr.markForCheck()" cardClass="trigger-card">
        ...
        <pui-card variant="flat" [clickable]="true" (cardClick)="confirmOpen=true;cdr.markForCheck()" cardClass="trigger-card">
          <div class="trigger-card__inner">
        ...
        <pui-card variant="flat" [clickable]="true" (cardClick)="successOpen=true;cdr.markForCheck()" cardClass="trigger-card">
          <div class="trigger-card__inner">
        ...
        <pui-card variant="flat" [clickable]="true" (cardClick)="formOpen=true;cdr.markForCheck()" cardClass="trigger-card">
          <div class="trigger-card__inner">
  ...
  <ng-container framework>
    ...
    <p class="fw-lead">Control visibility with <code>[open]</code>. Project body content directly inside the element; use <code>footer</code> attribute for footer actions. Listen to <code>(closed)</code> to sync your state when the backdrop or ✕ is clicked.</p>
    ...
    <div *ngIf="fwTab==='angular'" class="fw-panel">
      <div class="fw-note--angular">Import <code>ModalComponent</code> and bind <code>[open]</code> to a boolean. Close by setting it to <code>false</code> in the <code>(closed)</code> handler.</div>
      <pre><code>import &#123; ModalComponent, ButtonComponent &#125; from '&#64;solifi/platform-ui';
...
&#125;</code></pre>
    ...
    <div *ngIf="fwTab==='react'" class="fw-panel">
      <div class="fw-note--react">Set <code>open</code> and listen to <code>closed</code> custom event via <code>addEventListener</code> in a <code>useEffect</code>.</div>
...
function Page() &#123;
  ...
  return (
    &lt;&gt;
      &lt;pui-button variant="primary" onButtonclick=&#123;() =&gt; setOpen(true)&#125;&gt;Open&lt;/pui-button&gt;

      &lt;pui-modal ref=&#123;modalRef&#125; open=&#123;open ? '' : undefined&#125; title="Dialog" size="md"&gt;
        &lt;p&gt;Body content.&lt;/p&gt;
        &lt;div slot="footer"&gt;
          &lt;pui-button variant="secondary" onButtonclick=&#123;() =&gt; setOpen(false)&#125;&gt;Cancel&lt;/pui-button&gt;
          &lt;pui-button variant="primary"&gt;Confirm&lt;/pui-button&gt;
        &lt;/div&gt;
      &lt;/pui-modal&gt;
    &lt;/&gt;
  );
&#125;</code></pre>
    ...
    <div *ngIf="fwTab==='html'" class="fw-panel">
      <div class="fw-note--html">Set the <code>open</code> attribute/property to show the dialog. Listen to the <code>closed</code> custom event to sync your state.</div>
      <pre><code>&lt;pui-button id="btn-open" variant="primary"&gt;Open Modal&lt;/pui-button&gt;

&lt;pui-modal id="my-modal" title="Confirm" size="md"&gt;
  &lt;p&gt;Are you sure you want to proceed?&lt;/p&gt;
  &lt;div footer style="display:flex;gap:8px;justify-content:flex-end"&gt;
    &lt;pui-button id="btn-cancel" variant="secondary"&gt;Cancel&lt;/pui-button&gt;
    &lt;pui-button id="btn-confirm" variant="primary"&gt;Confirm&lt;/pui-button&gt;
  &lt;/div&gt;
&lt;/pui-modal&gt;

&lt;!-- Sizes: sm=360px · md=520px (default) · lg=720px --&gt;
&lt;pui-modal size="lg" title="Large Dialog"&gt;...&lt;/pui-modal&gt;
...
&lt;/script&gt;</code></pre>
    ...
    <h4 class="fw-ref-title">Modal Quick Reference</h4>
    <div class="xfw-wrap">
...
<!-- ══ Basic Modal ══ -->
<pui-modal [open]="basicOpen" title="Basic Dialog" size="md" (closed)="basicOpen=false;cdr.markForCheck()">
  ...
  <div footer class="modal-footer">
    <pui-button variant="secondary" (buttonClick)="basicOpen=false;cdr.markForCheck()">Cancel</pui-button>
    <pui-button variant="primary"   (buttonClick)="basicOpen=false;cdr.markForCheck()">Confirm</pui-button>
  </div>
</pui-modal>

<!-- ══ Confirm Delete Modal ══ -->
<pui-modal [open]="confirmOpen" title="Delete Item" size="sm" (closed)="confirmOpen=false;cdr.markForCheck()">
  <div class="modal-body-content">
    <pui-card variant="flat" size="sm" cardClass="danger-card">
      <div class="danger-inner">
        <div class="danger-icon"><pui-icon name="warning" size="lg" color="#b91c1c"></pui-icon></div>
        <div>
  ...
  <div footer class="modal-footer">
    <pui-button variant="secondary"   (buttonClick)="confirmOpen=false;cdr.markForCheck()">Cancel</pui-button>
    <pui-button variant="destructive" (buttonClick)="confirmOpen=false;cdr.markForCheck()">Yes, Delete</pui-button>
  </div>
</pui-modal>

<!-- ══ Success Modal ══ -->
<pui-modal [open]="successOpen" title="Payment Successful" size="sm" (closed)="successOpen=false;cdr.markForCheck()">
  <div class="modal-body-content modal-body-content--center">
    <div class="success-icon-wrap"><pui-icon name="check-circle" size="xl" color="#15803d"></pui-icon></div>
    ...
    <pui-card variant="flat" size="sm" style="width:100%;margin-top:16px">
      <div class="receipt-row"><span class="receipt-label">Transaction ID</span><span class="receipt-value">#TXN-20240610</span></div>
      <div class="receipt-row"><span class="receipt-label">Date</span><span class="receipt-value">06/10/2024</span></div>
      <div class="receipt-row"><span class="receipt-label">Status</span><span class="receipt-badge">Paid</span></div>
    </pui-card>
  </div>
  <div footer class="modal-footer modal-footer--center">
    <pui-button variant="primary" [fullWidth]="true" (buttonClick)="successOpen=false;cdr.markForCheck()">Done</pui-button>
  </div>
</pui-modal>

<!-- ══ Form Modal ══ -->
<pui-modal [open]="formOpen" title="Edit Profile" size="lg" (closed)="formOpen=false;cdr.markForCheck()">
  <div class="modal-body-content">
    <div class="form-grid">
      <pui-card variant="flat" size="sm"><div class="form-field"><label class="form-label">First Name</label><input class="form-input" type="text" placeholder="John"></div></pui-card>
      <pui-card variant="flat" size="sm"><div class="form-field"><label class="form-label">Last Name</label><input class="form-input" type="text" placeholder="Doe"></div></pui-card>
  ...
  <div footer class="modal-footer">
    <pui-button variant="secondary" (buttonClick)="formOpen=false;cdr.markForCheck()">Cancel</pui-button>
    <pui-button variant="primary"   (buttonClick)="formOpen=false;cdr.markForCheck()">Save Changes</pui-button>
  </div>
</pui-modal>
  `,
  styles: [`
    ...
    :host ::ng-deep .trigger-card { background:#f8fafc !important;border-color:#e5e7eb !important; }
    :host ::ng-deep .trigger-card:hover { border-color:#12C6A8 !important;background:#f0fdf9 !important;transform:none !important; }
    .trigger-card__inner { display:flex;align-items:center;gap:14px; }
    .trigger-icon { width:42px;height:42px;border-radius:12px;display:flex;align-items:center;justify-content:center;flex-shrink:0; }
    ...
    /* Danger card */
    :host ::ng-deep .danger-card { background:#fff5f5 !important;border-color:#fecaca !important; }
    ...
    .receipt-badge { font-size:11px;font-weight:700;padding:2px 10px;border-radius:999px;background:#dcfce7;color:#15803d;border:1px solid #15803d; }
    ...
    .form-input { width:100%;padding:8px 12px;border:1px solid #e5e7eb;border-radius:8px;font-size:13px;font-family:inherit;background:#fff;color:#111827;outline:none;box-sizing:border-box; }
