...
export class MultiSelectPageComponent {
  ...
  xfwRows = [
    { name: 'options',       angular: '[options]="opts"',         attr: "options='[â€¦]'",      js: 'el.options = [...]'      },
...
function ProjectSetup() {
  ...
  return (
    <pui-lib-multiselect
      ref={ref}
      placeholder="Pick your stackâ€¦"
      onValueChange={e => setStack(e.detail)}
    />
  );
}`;

  htmlCode = `<pui-lib-multiselect id="ms" placeholder="Select itemsâ€¦" searchable show-select-all></pui-lib-multiselect>
...
customElements.whenDefined('pui-lib-multiselect').then(() => {
  ...
  api: ApiRow[] = [
    ...
    { input: 'placeholder',   type: 'string',             default: '"Select optionsâ€¦"', description: 'Placeholder shown when no items are selected.' },
    ...
    { input: 'error',         type: 'string',             default: '""',              description: 'Error message â€” adds red border and displays below the trigger.' },
    { input: 'hint',          type: 'string',             default: '""',              description: 'Helper text shown below the trigger (hidden when error is set).' },
    { input: 'valueChange',   type: 'EventEmitter',       default: 'â€”',               description: 'Emits the updated selected-values array on every change.' },
    { input: 'change',        type: 'EventEmitter',       default: 'â€”',               description: 'Alias for valueChange â€” useful in React / HTML event listeners.' },
  ];
