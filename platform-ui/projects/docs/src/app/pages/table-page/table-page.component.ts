...
import { PuiTableComponent, TableColumn, TableAction } from '@bhairab-patra/platform-ui';
...
export class TablePageComponent {
  ...
  productActions: TableAction[] = [
    { label: 'Add Product', action: (row) => alert(`Add Product: ${row.name}`) },
    { label: 'Edit Product', action: (row) => alert(`Edit: ${row.name}`) },
    { label: 'Remove Product', action: (row) => alert(`Remove: ${row.name}`) },
  ];
  ...
  /* ── Action menu demo — Employee table ────────────── */
  actionEmpCols: TableColumn[] = [
    { key: 'name', label: 'Name', sortable: true, minWidth: '140px' },
    { key: 'department', label: 'Department', sortable: true, minWidth: '120px' },
  ...
  empActions: TableAction[] = [
    { label: 'Add Employee', action: (row) => alert(`Add Employee above: ${row.name}`) },
    { label: 'Edit Employee', action: (row) => alert(`Edit: ${row.name}`) },
    { label: 'Remove Employee', action: (row) => alert(`Remove: ${row.name}`), disabled: (row) => row.status === 'Inactive' },
  ];

  lastAction = '';

  onEmpAction(e: { action: TableAction; row: any }) {
    this.lastAction = `"${e.action.label}" → ${e.row.name}`;
    this.cdr.markForCheck();
  }
