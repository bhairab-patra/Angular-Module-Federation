...
import {
  PuiEditableTableComponent,
  TableColumn,
  TableAction,
  SortState,
  EditableRowSaveEvent,
  EditableRowEvent,
...
export class TableEditablePageComponent {
  ...
  onRowEdit(e: EditableRowEvent): void {
    this.editableLog = [
      `Editing row ${e.index + 1}: ${e.row.name}`,
      ...this.editableLog.slice(0, 4),
    ];
    this.cdr.markForCheck();
  }
  onSortChange(sort: SortState): void {
    this.editableLog = [
      `Sorted by ${sort.key || '—'} ${sort.dir || ''}`.trim(),
      ...this.editableLog.slice(0, 4),
    ];
    this.cdr.markForCheck();
  }
  ...
  openAddForm(): void {
    ...
    this.formSaveLabel = 'Add Employee';
    this.formData = {};
    this.formOpen = true;
    this.cdr.markForCheck();
  }
  ...
  openEditForm(row: Record<string, unknown>): void {
    ...
    this.formSaveLabel = 'Save Changes';
    this.formData = { ...row };
    this.formOpen = true;
    this.cdr.markForCheck();
  }
  ...
  onConfirmDelete(): void {
    if (this.confirmRow)
      this.formLog = [`Deleted: ${String(this.confirmRow['name'])}`, ...this.formLog.slice(0, 4)];
...
onSave(e: FormDialogSaveEvent) { console.log(e.data); this.formOpen = false; }`;
  ...
  xfwRows = [
    {
      name: 'columns',
      angular: '[columns]="colsArray"',
      attr: '— use JS property',
      js: 'el.columns = [{key,label,editable,sortable,...}]',
    },
    {
      name: 'data',
      angular: '[data]="rowsArray"',
      attr: '— use JS property',
      js: 'el.data = [{...},...]',
    },
    {
      name: 'heading',
      angular: 'heading="Team Members"',
      attr: 'heading="…"',
      js: 'el.heading = "…"',
    },
    {
    ...
    {
      name: 'striped',
      angular: '[striped]="bool"',
      attr: 'striped="true"',
      js: 'el.striped = true',
    },
    {
      name: 'stickyHeader',
      angular: '[stickyHeader]="true"',
      attr: 'sticky-header',
      js: 'el.stickyHeader = true',
    },
    {
      name: 'pagination',
      angular: '[pagination]="true"',
      attr: 'pagination',
      js: 'el.pagination = true',
    },
    {
      name: 'pageSize',
      angular: '[pageSize]="10"',
    ...
    },
    {
      ...
      angular: '[pageSizeOptions]="[10,25,50]"',
      attr: '— use JS property',
      js: 'el.pageSizeOptions = [10,25,50]',
    },
    {
      name: 'maxHeight',
      angular: '[maxHeight]="400"',
      attr: 'max-height="400"',
      js: 'el.maxHeight = 400',
    },
    {
      name: 'tooltipPosition',
      angular: '[tooltipPosition]="\'right\'"',
      attr: 'tooltip-position="right"',
      js: 'el.tooltipPosition = "right"',
    },
    {
      name: 'loading',
      angular: '[loading]="bool"',
      attr: 'loading="true"',
      js: 'el.loading = true',
    },
    {
      name: 'confirmDelete',
      angular: '[confirmDelete]="bool"',
      attr: 'confirm-delete="true"',
      js: 'el.confirmDelete = true',
    },
    {
      name: 'showToast',
      angular: '[showToast]="true"',
      attr: 'show-toast',
      js: 'el.showToast = true',
    },
    {
      name: 'saveToastMessage',
      angular: '[saveToastMessage]="\'…\'"',
      attr: 'save-toast-message="…"',
      js: 'el.saveToastMessage = "…"',
    },
    {
      name: 'deleteToastMessage',
      angular: '[deleteToastMessage]="\'…\'"',
      attr: 'delete-toast-message="…"',
      js: 'el.deleteToastMessage = "…"',
    },
    {
      name: 'showEditAction',
      angular: '[showEditAction]="bool"',
      attr: 'show-edit-action',
      js: 'el.showEditAction = false',
    },
    {
      name: 'showDeleteAction',
      angular: '[showDeleteAction]="bool"',
      attr: 'show-delete-action',
      js: 'el.showDeleteAction = false',
    },
    {
      name: 'rowActions',
      angular: '[rowActions]="actionsArray"',
      attr: '— use JS property',
      js: 'el.rowActions = [{label,icon,action,disabled}]',
    },
    {
      name: 'rowSave',
      angular: '(rowSave)="fn($event)"',
      attr: '— use addEventListener',
      js: 'el.addEventListener("rowSave", fn)',
    },
    {
      name: 'rowDelete',
      angular: '(rowDelete)="fn($event)"',
      attr: '— use addEventListener',
      js: 'el.addEventListener("rowDelete", fn)',
    },
    {
      name: 'rowEdit',
      angular: '(rowEdit)="fn($event)"',
      attr: '— use addEventListener',
      js: 'el.addEventListener("rowEdit", fn)',
    },
    {
      name: 'searchChange',
      angular: '(searchChange)="fn($event)"',
      attr: '— use addEventListener',
      js: 'el.addEventListener("searchChange", fn)',
    },
    {
      name: 'sortChange',
      angular: '(sortChange)="fn($event)"',
      attr: '— use addEventListener',
      js: 'el.addEventListener("sortChange", fn)',
    },
    {
      name: 'actionClick',
      angular: '(actionClick)="fn($event)"',
      attr: '— use addEventListener',
      js: 'el.addEventListener("actionClick", fn)',
    },
  ];
  ...
  api: ApiRow[] = [
    ...
    },
    {
      ...
      default: "''",
      description:
        'Optional title shown on the left of the toolbar. The toolbar also appears (title-less) whenever searchable is true.',
    },
    {
      input: 'searchable',
      type: 'boolean',
      default: 'false',
      description:
        'Shows a pui-lib-search box in the toolbar that filters rows by matching any column value, live as you type. Edit/delete/save/sort always act on the correct original row even while filtered.',
    },
    {
      input: 'striped',
      type: 'boolean',
      default: 'true',
      description:
        'Alternating row background shading. Same input name/behavior as pui-lib-table and pui-lib-data-table.',
    },
    {
      input: 'stickyHeader',
      type: 'boolean',
      default: 'true',
      description:
        'Keeps the header row pinned while the body scrolls. Set false to let it scroll away with the content.',
    },
    {
      input: 'pagination',
      type: 'boolean',
      default: 'false',
      description:
        'Paginates rows (after search/sort) using pui-lib-simple-pagination in the footer, instead of showing every row in one scroll.',
    },
    {
      input: 'pageSize',
      type: 'number',
      default: '10',
      description: 'Rows per page when pagination is true.',
    },
    {
      input: 'pageSizeOptions',
      type: 'number[]',
      default: '[10, 25, 50, 100]',
      description: 'Options shown in the pagination page-size selector.',
    },
    {
      ...
      default: `'right'`,
      description:
        'Placement of the tooltip that appears when a truncated header or cell is hovered.',
    },
    {
      input: 'maxHeight',
      type: 'number',
      default: '480',
      description: 'Max height (px) of the scroll container',
    },
    {
      input: 'loading',
      type: 'boolean',
      default: 'false',
      description: 'Shows a loading skeleton overlay',
    },
    {
      input: 'confirmDelete',
      type: 'boolean',
      default: 'false',
      description: 'Shows a built-in confirm dialog before deleting a row',
    },
    {
      input: 'showToast',
      type: 'boolean',
      default: 'true',
      description:
        'Shows a toast (via the shared ToastService) after a successful save or delete. Mount <pui-lib-toast-container> once at the app root for toasts to render.',
    },
    {
      input: 'saveToastMessage',
      type: 'string',
      default: "'Row saved successfully'",
      description: 'Toast message shown after a successful save.',
    },
    {
      input: 'deleteToastMessage',
      type: 'string',
      default: "'Row deleted successfully'",
      description: 'Toast message shown after a row is deleted.',
    },
    {
      input: 'showEditAction',
      type: 'boolean',
      default: 'true',
      description:
        'Shows/hides the built-in Edit (pencil) button independently of Delete — e.g. set false for a view/delete-only row toolbar.',
    },
    {
      input: 'showDeleteAction',
      type: 'boolean',
      default: 'true',
      description: 'Shows/hides the built-in Delete (trash) button independently of Edit.',
    },
    {
      input: 'rowActions',
      type: 'TableAction[]',
      default: '[]',
      description:
        'Extra per-row buttons shown alongside Edit/Delete (e.g. "View") — same TableAction shape as pui-lib-table / pui-lib-data-table: { label, icon?: rawSvgString, action(row), disabled?(row) }. Hidden entirely while any row is being edited. The whole Actions column (header + cell) disappears automatically when showEditAction, showDeleteAction, and rowActions are all off/empty — nothing to configure separately for that.',
    },
    {
      input: 'rowSave',
      type: 'EventEmitter<EditableRowSaveEvent>',
      default: '—',
      description:
        'Emits {index, row, oldRow} when a row is saved (only after all validation rules pass)',
    },
    {
      input: 'rowDelete',
      type: 'EventEmitter<EditableRowEvent>',
      default: '—',
      description: 'Emits {index, row} after a row is deleted',
    },
    {
      input: 'rowEdit',
      type: 'EventEmitter<EditableRowEvent>',
      default: '—',
      description: 'Emits {index, row} when a row enters edit mode',
    },
    {
      input: 'searchChange',
      type: 'EventEmitter<string>',
      default: '—',
      description: 'Emits the current search term on every keystroke',
    },
    {
      input: 'sortChange',
      type: 'EventEmitter<SortState>',
      default: '—',
      description:
        'Emits {key, dir} whenever the sort column/direction changes (dir cycles asc → desc → none)',
    },
    {
      input: 'actionClick',
      type: 'EventEmitter<{action,row,index}>',
      default: '—',
      description:
        "Emits whenever a rowActions button is clicked, in addition to that action's own action(row) callback firing.",
    },
  ];
  ...
  formDialogApi: ApiRow[] = [
    ...
    { input: 'title', type: 'string', default: "''", description: 'Dialog heading text' },
    {
      ...
      default: '[]',
      description: 'Field definitions: key, label, type, required, placeholder, options, span',
    },
    {
      input: 'data',
      type: 'Record<string,any>',
      default: '{}',
      description: 'Pre-filled values for edit mode; empty for add',
    },
    ...
    },
    {
      ...
      type: "'sm'|'md'|'lg'",
      default: "'md'",
      description: 'Dialog width: sm=400px, md=560px, lg=720px',
    },
    {
    ...
    {
      input: 'save',
      type: 'EventEmitter<FormDialogSaveEvent>',
      default: '—',
      description: 'Emits {data} with all field values on save',
    },
    {
      input: 'cancel',
      type: 'EventEmitter<void>',
      default: '—',
      description: 'Emits when cancel is clicked',
    },
    {
      input: 'closed',
      type: 'EventEmitter<void>',
      default: '—',
      description: 'Emits when the dialog closes (any method)',
    },
  ];
  ...
  confirmDialogApi: ApiRow[] = [
    ...
    },
    {
      ...
      type: 'EventEmitter<void>',
      default: '—',
      description: 'Emits when the user confirms',
    },
    {
      input: 'cancelled',
      type: 'EventEmitter<void>',
      default: '—',
      description: 'Emits when the user cancels',
    },
    {
      input: 'closed',
      type: 'EventEmitter<void>',
