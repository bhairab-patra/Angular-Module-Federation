...
export class FormDialogPageComponent {
  ...
  openAdd(): void {
    this.basicTitle = 'Add User';
    this.basicSaveLabel = 'Add User';
    this.basicData = {};
    this.basicOpen = true;
    this.cdr.markForCheck();
  }
  openEdit(): void {
    this.basicTitle = 'Edit User';
    this.basicSaveLabel = 'Save Changes';
    this.basicData = {
      firstName: 'Alice',
      lastName: 'Johnson',
      email: 'alice@example.com',
      role: 'admin',
      bio: 'Frontend Engineer at Platform UI.',
    };
    this.basicOpen = true;
    this.cdr.markForCheck();
  }
  ...
  onBasicSave(e: FormDialogSaveEvent): void {
    this.basicLog = [
      `Saved: ${String(e.data['firstName'])} ${String(e.data['lastName'])} (${String(e.data['role'])})`,
      ...this.basicLog.slice(0, 3),
    ];
  ...
  onProductSave(e: FormDialogSaveEvent): void {
    this.productLog = [
      `Saved: ${String(e.data['name'])} — $${String(e.data['price'])}`,
      ...this.productLog.slice(0, 3),
    ];
...
export class MyComponent {
  ...
  api: ApiRow[] = [
    ...
    { input: 'title', type: 'string', default: "''", description: 'Dialog heading text' },
    {
      ...
      default: '[]',
      description: 'Field definitions — see FormDialogField below',
    },
    {
      input: 'data',
      type: 'Record<string,any>',
      default: '{}',
      description: 'Pre-filled values for edit mode; pass empty object for add',
    },
    {
      input: 'saveLabel',
      type: 'string',
      default: "'Save'",
      description: 'Primary (save) button label',
    },
    {
      input: 'cancelLabel',
      type: 'string',
    ...
    },
    {
      ...
      type: "'sm'|'md'|'lg'",
      default: "'md'",
      description: 'Dialog max-width: sm = 400 px, md = 560 px, lg = 720 px',
    },
    {
    ...
    },
    {
      ...
      default: '—',
      description: 'Emits { data } with all field values when Save is clicked',
    },
    {
      input: 'cancel',
    ...
    },
    {
      ...
      type: 'EventEmitter<void>',
      default: '—',
      description: 'Emits when the dialog fully closes (any method)',
    },
  ];
  ...
  fieldApi: ApiRow[] = [
    {
      ...
      type: 'string',
      default: '—',
      description: 'Field key — maps to the data object property',
    },
    {
      input: 'label',
      type: 'string',
      default: '—',
      description: 'Field label text shown above the input',
    },
    {
      input: 'type',
    ...
    { input: 'placeholder', type: 'string', default: "''", description: 'Input placeholder text' },
    {
      ...
      type: 'boolean',
      default: 'false',
      description: 'Marks the field as required and validates before save',
    },
    {
      input: 'readonly',
      type: 'boolean',
      default: 'false',
      description: 'Renders the field as read-only',
    },
    {
      input: 'options',
      type: 'SelectOption[]',
      default: '[]',
      description: 'Dropdown options — used when type is "select"',
    },
    {
      input: 'span',
      type: "'half'|'full'",
      default: "'full'",
      description: 'Column span in the 2-column grid layout',
    },
  ];
}
