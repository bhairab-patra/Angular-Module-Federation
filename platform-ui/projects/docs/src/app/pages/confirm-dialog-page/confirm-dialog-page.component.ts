...
export class ConfirmDialogPageComponent {
  ...
  onDeleteConfirm(): void {
    this.deleteResult = '✓ Record deleted.';
    this.deleteOpen = false;
    this.cdr.markForCheck();
  }
  onDeleteCancel(): void {
    this.deleteResult = '✕ Cancelled.';
    this.deleteOpen = false;
    this.cdr.markForCheck();
  }
  ...
  onInfoConfirm(): void {
    this.infoResult = '✓ Changes published.';
    this.infoOpen = false;
    this.cdr.markForCheck();
  }
  onInfoCancel(): void {
    this.infoResult = '✕ Kept as draft.';
    this.infoOpen = false;
    this.cdr.markForCheck();
  }
  ...
  onLogoutCancel(): void {
    this.logoutResult = '✕ Stayed logged in.';
    this.logoutOpen = false;
    this.cdr.markForCheck();
  }
  ...
  onArchiveConfirm(): void {
    this.archiveResult = '✓ Project archived.';
    this.archiveOpen = false;
    this.cdr.markForCheck();
  }
  onArchiveCancel(): void {
    this.archiveResult = '✕ Cancelled.';
    this.archiveOpen = false;
    this.cdr.markForCheck();
  }
...
export class MyComponent {
  ...
  api: ApiRow[] = [
    ...
    { input: 'title', type: 'string', default: "'Confirm?'", description: 'Dialog heading text' },
    {
      ...
      type: 'string',
      default: "''",
      description: 'Body message text shown below the title',
    },
    {
      input: 'confirmLabel',
      type: 'string',
      default: "'Confirm'",
      description: 'Primary (confirm) button label',
    },
    {
      input: 'cancelLabel',
      type: 'string',
    ...
    },
    {
      ...
      default: "'destructive'",
      description:
        'Variant applied to the confirm button: primary, primary-outline, secondary, destructive, etc.',
    },
    {
      input: 'tone',
      type: "'danger' | 'warning'",
      default: "'danger'",
      description:
        'Color of the icon badge — danger (red) for destructive actions like delete, warning (amber) for reversible-but-risky actions like discarding unsaved changes.',
    },
    {
    ...
    },
    {
      ...
      default: '—',
      description: 'Emits when the user clicks the confirm button',
    },
    {
      input: 'cancelled',
      type: 'EventEmitter<void>',
      default: '—',
      description: 'Emits when the user clicks cancel or presses Escape',
    },
    {
      input: 'closed',
      type: 'EventEmitter<void>',
      default: '—',
      description: 'Emits when the dialog closes by any method',
    },
  ];
}
