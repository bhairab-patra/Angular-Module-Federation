...
export class ToastPageComponent {
  ...
  showSuccess(): void {
    this.toastSvc.success('Your changes have been saved successfully.', { title: 'Changes Saved' });
  }
  showError(): void {
    this.toastSvc.error('Unable to save your changes. Please try again.', { title: 'Error Occurred' });
  }
  showWarning(): void {
    this.toastSvc.warning('Your subscription will expire in 3 days.', { title: 'Action Required' });
  }
  showInfo(): void {
    this.toastSvc.info('12 new products have been added to your inventory.', { title: 'Inventory Updated' });
  }
  showWithTitle(): void {
    this.toastSvc.success('Your profile has been updated successfully.', { title: 'Profile Saved' });
  }
  showWithAction(): void {
  ...
  showPersistent(): void {
    this.toastSvc.info('This notification will stay until dismissed.', { title: 'Persistent', duration: 0 });
  }

  trackByIndex(_i: number): number { return _i; }

  at(pos: string): void {
    this.toastSvc.info(`Toast at ${pos}`, { position: pos as ToastPosition });
  }
  dur(ms: number): void {
    this.toastSvc.success(`Auto-dismiss in ${ms / 1000}s`, { duration: ms });
  }
...
export class ProductsComponent {
  ...
  ngOnInit() {
    this.toast.configure({ position: 'top-right', duration: 4000 });
  }
}`;
...
<body>
  ...
  api: ApiRow[] = [
    ...
    { input: 'ToastConfig.type',            type: "'success'|'error'|'warning'|'info'", default: "'info'", description: 'Semantic type — controls icon, accent bar, and background tint.' },
