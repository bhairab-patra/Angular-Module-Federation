...
export class InputPageComponent {
  ...
  xfwRows = [
    ...
    { name: 'minLength',   angular: '[minLength]="6"',                   attr: 'min-length="6"',        js: 'el.minLength = 6' },
    { name: 'noNumbers',   angular: '[noNumbers]="bool"',                attr: '— use JS property',     js: 'el.noNumbers = true' },
    { name: 'noSpaces',    angular: '[noSpaces]="bool"',                 attr: '— use JS property',     js: 'el.noSpaces = true' },
    { name: 'lettersOnly', angular: '[lettersOnly]="bool"',              attr: '— use JS property',     js: 'el.lettersOnly = true' },
    { name: 'textCase',    angular: 'textCase="upper|lower|any"',        attr: 'text-case="upper"',     js: 'el.textCase = "upper"' },
    { name: 'pattern',     angular: 'pattern="^[0-9]{6}$"',              attr: 'pattern="^[0-9]{6}$"',  js: 'el.pattern = "^[0-9]{6}$"' },
    { name: 'validateEmail', angular: '[validateEmail]="bool"',          attr: '— use JS property',     js: 'el.validateEmail = true' },
    ...
    { name: 'validityChange', angular: '(validityChange)="fn($event)"',  attr: '— use addEventListener', js: 'el.addEventListener("validityChange", fn)' },
  ];
...
<script>
  ...
  api: ApiRow[] = [
    ...
    { input: 'autocomplete',type: 'string',           default: "'off'", description: 'Native autocomplete attribute — defaults to off so the browser never suggests/autofills a previously-entered value for this field.' },
    { input: 'minLength',   type: 'number | null',    default: 'null',  description: 'Minimum character count — a real validator (unlike maxLength, which only caps typing). Produces the same {minlength:{requiredLength,actualLength}} shape Validators.minLength does.' },
    { input: 'noNumbers',   type: 'boolean',          default: 'false', description: 'Rejects any digit 0-9 in the value — e.g. for a name field.' },
    { input: 'noSpaces',    type: 'boolean',          default: 'false', description: 'Rejects whitespace anywhere in the value — e.g. for a username or code field.' },
    { input: 'lettersOnly', type: 'boolean',          default: 'false', description: 'Only a-z/A-Z allowed — no digits, symbols, or spaces. Combine with textCase to also pin the letters to one case.' },
    { input: 'textCase',    type: `'upper'|'lower'|'any'`, default: "'any'", description: 'Requires every letter in the value to be a specific case. \'any\' applies no restriction.' },
    { input: 'pattern',     type: 'string',           default: "''",    description: 'Escape hatch for anything minLength/noNumbers/noSpaces/lettersOnly/textCase don\'t cover — a regex the value must match. Shows a built-in "doesn\'t match the required format" error after blur unless you\'ve set your own [error]. Also registered as a real Angular Validator, so formControlName picks up the same pattern error in form.get(\'x\').errors.' },
    { input: 'validateEmail', type: 'boolean',        default: "type === 'email'", description: 'Turns on built-in email-format validation. On automatically when type="email" — set explicitly to force it on for a non-email type field, or off to disable it.' },
    ...
    { input: 'validityChange', type: 'EventEmitter<boolean>', default: '—', description: 'Emits the field\'s own pattern/email validity whenever it changes — fires even with no surrounding Angular form (plain HTML/React usage included).' },
  ];
}
